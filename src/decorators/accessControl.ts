    ...
    descriptor.value = async function (this: any, ...args: any[]) {
      ...
      try {
        ...
        if (logger) {
          ...
          if (!msg)
            msg = `Access attempt: user=${ctx.user?.id}, method=${
              ctx.method
        ...
        if (logger) {
          ...
          if (typeof msg === "function") msg = msg(ctx);
          if (!msg)
            msg = `Access denied: user=${ctx.user?.id}, method=${ctx.method}, error=${error?.message}`;
    ...
    descriptor.value = async function (this: any, ...args: any[]) {
      ...
      for (const action of actions) {
        for (const resource of resourceList) {
          const result = await iam.can({ user: this.user, action, resource });
          if (result.decision)
            throw new Error("Access denied: explicitly denied action");
        }
      }
      return original.apply(this, args);
    };
...
export function AccessCondition(
  condition: (user: any, ...args: any[]) => boolean
): MethodDecorator {
  return (target, propertyKey, descriptor: PropertyDescriptor) => {
    const original = descriptor.value;
...
export function AccessControl(
  params: Partial<CanParams> | ((...args: any[]) => Partial<CanParams>)
): MethodDecorator {
  return (target, propertyKey, descriptor: PropertyDescriptor) => {
    const original = descriptor.value;
    ...
    descriptor.value = function (this: any, ...args: any[]) {
      ...
      if (!iam) throw new Error("IAM instance not found");
      const canParams =
        typeof params === "function" ? params.bind(this)(...args) : params;
